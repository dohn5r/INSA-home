...
load("../Appendix B - Data pre-processing/DATA/datasets/ema_finalClean.RData")
load("../Appendix B - Data pre-processing/DATA/datasets/demos_clean.RData")
...
ema$SO.num <- as.POSIXct(paste(hour(ema$SO),minute(ema$SO)),format="%H %M",tz="GMT") 
ema$WakeUp.num <- as.POSIXct(paste(hour(ema$WakeUp),minute(ema$WakeUp)),format="%H %M",tz="GMT")
...
Here, we create the variable `diary.nextDay` to mark those cases in which daily diaries were **filled on the following day**, that is we we mark the cases with `dailyDiary` `Startdate` after `sleepLog` `EndTime`, and those with valid `dailyDiary` but invalid `sleepLog` data with a `Startdate` later than 6 AM.
```{r fig.width=10,fig.height=2.5,warning=FALSE,message=FALSE}
# marking cases with Startdate > EndTime
ema[!is.na(ema$dailyStress) & !is.na(ema$TIB),c("diary.nextDay","diary.nextDay2")] <- 0
ema[!is.na(ema$dailyStress) & !is.na(ema$TIB) & ema$Startdate>ema$EndTime,"diary.nextDay"] <- 1
cat(nrow(ema[!is.na(ema$diary.nextDay) & ema$diary.nextDay==1,]),"diary responses entered after wake up (",
    round(100*nrow(ema[!is.na(ema$diary.nextDay) & ema$diary.nextDay==1,])/nrow(ema[!is.na(ema$Startdate),]),1),"% )")

# marking cases with Startdate > 6 AM and < 8 PM
ema$StartedHour <- as.POSIXct(paste(hour(ema$Startdate),minute(ema$Startdate)),format="%H %M",tz="GMT") # isolating time
...
cat(nrow(ema[!is.na(ema$diary.nextDay2) & ema$diary.nextDay2==1,]),"diary responses entered after 6 AM (",
    round(100*nrow(ema[!is.na(ema$diary.nextDay2) & ema$diary.nextDay2==1,])/nrow(ema[!is.na(ema$Startdate),]),1),
...
# showing cases with Startdate > 6 AM and diary.nextDay = 0
ema[!is.na(ema$diary.nextDay2) & ema$diary.nextDay2==1 & !is.na(ema$diary.nextDay) & ema$diary.nextDay==0,
    c("ID","ActivityDate","StartTime","EndTime","Startdate","diary.nextDay")]
...
35. `Startdate` = initiation hour of the diary form (in "mm-dd-yyyy hh:mm:ss" format)
...
# round(fitInd(model=list(fit.null,fit.ind),
#               models.names=c("Lv2 Null","Lv2 Independence")),2)
...
ema[!is.na(ema$Startdate),"fs.w"] <- as.numeric(lavPredict(fit.conf.fix,level=1))
...
```{r warning=FALSE,message=FALSE,fig.width=10,fig.height=4,error = T}
...
multidesc <- function(long,wide,cluster="S.ID",lv1,lv2,group,group.labels=NA){ require(Rmisc); require(lme4)
  ...
  out <- data.frame(Measure=lv1[1],
                    N=Rmisc::summarySE(long,lv1[1],na.rm=TRUE)[,2],
                    Mean=paste(round(Rmisc::summarySE(long,lv1[1],na.rm=TRUE)[,3],2)," (",
                               round(Rmisc::summarySE(long,lv1[1],na.rm=TRUE)[,4],2),")",sep=""))
  if(!is.na(group)){
    ...
    out$Mean1 = paste(round(Rmisc::summarySE(long[long$group==groups[1],],lv1[1],na.rm=TRUE)[,3],2)," (",
                      round(Rmisc::summarySE(long[long$group==groups[1],],lv1[1],na.rm=TRUE)[,4],2),")",sep="")
    out$Mean2 = paste(round(Rmisc::summarySE(long[long$group==groups[2],],lv1[1],na.rm=TRUE)[,3],2)," (",
                      round(Rmisc::summarySE(long[long$group==groups[2],],lv1[1],na.rm=TRUE)[,4],2),")",sep="") }
  
  for(i in 2:length(lv1)){
    if(!is.na(group)){
      out <- rbind(out,
                 data.frame(Measure=lv1[i],
                            N=Rmisc::summarySE(long,lv1[i],na.rm=TRUE)[,2],
                            Mean=paste(round(Rmisc::summarySE(long,lv1[i],na.rm=TRUE)[,3],2)," (",
                                       round(Rmisc::summarySE(long,lv1[i],na.rm=TRUE)[,4],2),")",sep=""),
                            Mean1=paste(round(Rmisc::summarySE(long[long$group==groups[1],],lv1[i],na.rm=TRUE)[,3],2)," (",
                                        round(Rmisc::summarySE(long[long$group==groups[1],],lv1[i],na.rm=TRUE)[,4],2),")",sep=""),
                            Mean2=paste(round(Rmisc::summarySE(long[long$group==groups[2],],lv1[i],na.rm=TRUE)[,3],2)," (",
                                        round(Rmisc::summarySE(long[long$group==groups[2],],lv1[i],na.rm=TRUE)[,4],2),")",sep="")))
    } else {
      out <- rbind(out,
                 data.frame(Measure=lv1[i],
                            N=Rmisc::summarySE(long,lv1[i],na.rm=TRUE)[,2],
    ...
    if(!is.na(group)){
      out <- rbind(out,
                   data.frame(Measure=lv2[i],
                              N=Rmisc::summarySE(wide,lv2[i],na.rm=TRUE)[,2],
                              Mean=paste(round(Rmisc::summarySE(wide,lv2[i],na.rm=TRUE)[,3],2)," (",
                                         round(Rmisc::summarySE(wide,lv2[i],na.rm=TRUE)[,4],2),")",sep=""),
                              Mean1=paste(round(Rmisc::summarySE(wide[wide$group==groups[1],],lv2[i],na.rm=TRUE)[,3],2)," (",
                                          round(Rmisc::summarySE(wide[wide$group==groups[1],],lv2[i],na.rm=TRUE)[,4],2),")",sep=""),
                              Mean2=paste(round(Rmisc::summarySE(wide[wide$group==groups[2],],lv2[i],na.rm=TRUE)[,3],2)," (",
                                          round(Rmisc::summarySE(wide[wide$group==groups[2],],lv2[i],na.rm=TRUE)[,4],2),")",sep="")))
    } else {
      out <- rbind(out,
                 data.frame(Measure=lv2[i],
                            N=Rmisc::summarySE(wide,lv2[i],na.rm=TRUE)[,2],
  ...
  if(!is.na(group)){ if(!all(is.na(group.labels))){
...
multicorr <- function(long,wide,lv1,lv2,cluster="ID"){ require(Hmisc); require(Rmisc)
  ...
  for(VAR in lv1){
    wide <- cbind(wide,newVar=Rmisc::summarySE(long,VAR,"ID",na.rm=TRUE)[,3])
...
```{r fig.width=12,fig.height=9,message=FALSE,warning=FALSE,error = T}
desc <- cbind(desc,
...
```{r warning=FALSE,message=FALSE,fig.width=12,fig.height=7,error = T}
...
```{r fig.width=10,fig.height=6,message=FALSE,warning=FALSE,error =T}
# mean centering PsyDist
demos$PsyDist.cm <- Rmisc::summarySE(ema,measurevar="PsyDist",groupvars="ID",na.rm=TRUE)[,3]
...
##### s.archit
```{r echo=TRUE,fig.width=5,fig.height=4,warning=FALSE, error = T}
for(Var in s.archit){ 
...
```{r echo=TRUE,fig.width=11,fig.height=6,warning=FALSE,message=FALSE,error=T}
# plotting
...
Rmisc::summarySE(ema,measurevar="stress",na.rm=TRUE)[,1:4] # stress in the sample
Rmisc::summarySE(ema,measurevar="stress",groupvars="sex",na.rm=TRUE)[,1:4] # stress by sex
Rmisc::summarySE(ema,measurevar="stress",groupvars="insomnia",na.rm=TRUE)[,1:4] # stress by insomnia
Rmisc::summarySE(ema,measurevar="stress",groupvars="insomnia.group",na.rm=TRUE)[,1:4] # stress by insomnia.group
...
# computing mean and SD in the sample and by group
Rmisc::summarySE(ema,measurevar="worry",na.rm=TRUE)[,1:4] # worry in the sample
Rmisc::summarySE(ema,measurevar="worry",groupvars="sex",na.rm=TRUE)[,1:4] # worry by sex
Rmisc::summarySE(ema,measurevar="worry",groupvars="insomnia",na.rm=TRUE)[,1:4] # worry by insomnia
Rmisc::summarySE(ema,measurevar="worry",groupvars="insomnia.group",na.rm=TRUE)[,1:4] # worry by insomnia.group
...
# computing mean and SD in the sample and by group
Rmisc::summarySE(ema,measurevar="mood",na.rm=TRUE)[,1:4] # mood in the sample
Rmisc::summarySE(ema,measurevar="mood",groupvars="sex",na.rm=TRUE)[,1:4] # mood by sex
Rmisc::summarySE(ema,measurevar="mood",groupvars="insomnia",na.rm=TRUE)[,1:4] # mood by insomnia
Rmisc::summarySE(ema,measurevar="mood",groupvars="insomnia.group",na.rm=TRUE)[,1:4] # mood by insomnia.group
