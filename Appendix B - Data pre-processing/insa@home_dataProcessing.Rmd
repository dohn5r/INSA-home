...
multidata.read <- function(data.path,idChar,groupChar=NA,nSubj=NA,surveySparrow=FALSE){ 
  ...
  if (surveySparrow & length(paths) == 1){
    data <- read.csv(paste(data.path,paths[[1]],sep="/"))
    data <- dplyr::rename_with(data, stringr::str_to_sentence) %>%
      dplyr::rename_with(~gsub("^X[0-9]+", "", .), everything()) %>%
      dplyr::rename_with(., ~stringr::str_replace_all(., '\\.', '')) %>%
      na.omit() %>%
      dplyr::mutate(ID = Recordid,
                      ID = as.factor(paste(substr(Recordid,idChar[1],idChar[2]),sep=""))) %>%
      dplyr::relocate(ID) %>%
      dplyr::select(-c("Recordid")) %>%
      dplyr::mutate(Startdate = sub("\\..*", "", Startdate),
                    Enddate = sub("\\..*", "", Enddate)) %>%
      dplyr::select(-starts_with("Repeat"))

  }else{
    # iteratively reading each file and adding it to the same data.frame
    for(path in paths){ 
      new.data <- read.csv(paste(data.path,path,sep="/"))
      new.data$ID <- as.factor(paste(
        substr(path,idChar[1],idChar[2]),sep="")) # participant's identifier from file name
      ...
      if(surveySparrow & length(paths) > 1){
        ...
        if(ncol(new.data)==40){ 
          new.data[,c("SubmissionId","TimeZone","DeviceType","BrowserLanguage")] <- NULL }
        ...
        if(new.data[1,"ID"]=="s095"){ 
          colnames(new.data)[32:33] <- colnames(data)[32:33] } # fixing wrongly encoded col names
...
dailyAct <- multidata.read(data.path="DATA/Fitbit All Daily Activity",idChar=c(1,6),groupChar=c(8,11),nSubj=5)

# Fitbit - hourlySteps (143,961 hours from 93 participants)
hourlySteps <- multidata.read(data.path="DATA/Fitbit Hourly Steps",idChar=c(1,6),groupChar=c(8,11),nSubj=5)

# Fitbit - sleepLog (5,403 nights from 93 participants)
sleepLog <- multidata.read(data.path="DATA/Fitbit Sleep Log",idChar=c(1,6),groupChar=c(8,11),nSubj=5)

# Fitbit - sleepEBE (4,243,037 epochs from 93 participants)
sleepEBE <- multidata.read(data.path="DATA/Fitbit Sleep Stages (30sec)",idChar=c(1,6),groupChar=c(8,11),nSubj=5)

# Fitbit - classicEBE (2,505,034 epochs from 93 participants)
classicEBE <- multidata.read(data.path="DATA/Fitbit Sleep Classic (1min)",idChar=c(1,6),groupChar=c(8,11),nSubj=5) 

# Fitbit - HR.1min (6,986,307 epochs from 93 participants)
HR.1min <- multidata.read(data.path="DATA/Fitbit HR (1min)",idChar=c(1,6),groupChar=c(8,11),nSubj=5)

# Survey Sparrow - dailyDiary (5,133 daily responses from 93 participants)
dailyDiary <- multidata.read(data.path="DATA/Survey Sparrows",idChar=c(1,6),nSubj=5,surveySparrow=TRUE)
```
...
demos = read.csv("/Users/dohn5r/Documents/INSA-home/Appendix B - Data pre-processing/DATA/demographics.csv")
#load("/Users/dohn5r/Documents/INSA-home/Appendix D - Data/demosFINAL.RData")
#load("/Users/dohn5r/Documents/INSA-home/Appendix D - Data/emaFINAL.RData")
```
...
timeCheck <- function(data,ID="ID",day="ActivityDate",hour=NA,returnInfo=FALSE,printInfo=TRUE,
                      input.dayFormat="%m/%d/%Y",output.dayFormat="%Y-%m-%d",output.hourFormat="%m/%d/%Y %H:%M %p",
  ...
  if(!is.na(hour)){ # setting hour format
    ...
    if(is.na(day) | is.null(data$day[1])){ # when hour but not day is specified, day is computed from hour
      ...
      if(!is.na(hour) & !is.na(LogId) & day.withinNight==TRUE){ # date as the first epoch's date
        for(LOG in levels(data$LogId)){ 
          data[data$LogId==LOG,"day"] <- data[data$LogId==LOG,"day"][1]
        }
      }
    }
    }
...
hourlySteps <-  timeCheck(data=hourlySteps,ID="ID",day="ActivityDate",hour="ActivityHour",
                          input.dayFormat="%m/%d/%Y",output.dayFormat="%Y-%m-%d",output.hourFormat="%m/%d/%Y %I:%M:%S %p")
```
...
sleepLog$lastTimeChar <- substr(sleepLog$StartTime,nchar(sleepLog$StartTime),nchar(sleepLog$StartTime)+1)
...
if (sum(summary(sleepLog$LogId)==2) > 0){
...
```{r eval = shortNaps_eval, message=FALSE,warning=FALSE,fig.height=4,fig.width=10, error = T}
...
DST.changes <- as.Date(c("2021-03-14","2021-11-07", "2022-03-13","2022-11-06","2023-03-12","2023-11-05"))
...
```{r fig.width=12,fig.height=4}
# setting DST changing times
DST.changes <- as.POSIXct(paste(c("2021-03-14","2021-11-07", "2022-03-13","2022-11-06","2023-03-12","2023-11-05"),
                                "02:00:00",sep=" "))
...
- only in one case (`LogId` = 39430330016) there are two consecutive epochs separated by more than 60 seconds, namely 1 hour, and this case is observed precisely on **March 8th, 2020** (DST change)
...
- now, no more cases have one or more couples of consecutive epochs differing more than 30 sec, with the only exception of *SleepLog* 39430330016 (see section 2.5.3)
...
```{r fig.width=12,fig.height=4}
# setting DST changing times
DST.changes <- as.POSIXct(paste(c("2021-03-14","2021-11-07", "2022-03-13","2022-11-06","2023-03-12","2023-11-05"),sep=" "),tz="GMT")
...
classicEBE <- classicEBE[order(classicEBE$ID,classicEBE$ActivityDate,classicEBE$Time, classicEBE$LogId),]
...
- none of the cases shows time shifts longer than 60 secs, including case `LogId` = 39430330016, which showed a shift of 1 hour between 7:59 and 9:00 in sleep EBE data

<br>

Here, we better inspect `sleepLog`, `sleepEBE`, and `classicEBE` times for this case. 
```{r warning=FALSE,message=FALSE}
# computing and showing sleep times for LogId 39430330016
times <- data.frame(dataType=c("LogId","sleepEBE","classicEBE"),
           start=c(sleepLog[sleepLog$LogId=="39430330016","StartTime"],head(sleepEBE[sleepEBE$LogId=="39430330016","Time"],1),
                   head(classicEBE[classicEBE$LogId=="39430330016","Time"],1)),
           end=c(sleepLog[sleepLog$LogId=="39430330016","EndTime"],tail(sleepEBE[sleepEBE$LogId=="39430330016","Time"],1),
                 tail(classicEBE[classicEBE$LogId=="39430330016","Time"],1)),
           duration=c(sleepLog[sleepLog$LogId=="39430330016","TimeInBed"],nrow(sleepEBE[sleepEBE$LogId=="39430330016",])/2,
                      nrow(classicEBE[classicEBE$LogId=="39430330016",])))
...
Here, we discard `LogId` 39430330016 epochs from `sleepEBE`.
```{r warning=FALSE,message=FALSE}
sleepEBE <- sleepEBE[sleepEBE$LogId!="39430330016",] # removing case from sleepEBE
sleepEBE$LogId <- as.factor(as.character(sleepEBE$LogId)) # resetting LogIds
ClassicAndSleepLog <- c(ClassicAndSleepLog,"39430330016")
```
...
`dailyDiary` data were recorded with Survey Sparrow (SurveySparrow Inc.), and include the daily diary reports on psychological distress and other psychosocial variables self-reported each evening by participants. dailyDiary data are stored in a dataset with one row per day, with the `Startdate` and `Enddate` variables indicating the survey start and submission time, respectively. Thus, in this dataset we only need to recode the `Startdate` variable, based on which the .
```{r message=FALSE, error=F}
# recoding day and hour, and checking time and missing data points
dailyDiary <-  timeCheck(data=dailyDiary,ID="ID",day="ActivityDate",hour="Startdate",                         input.dayFormat="%Y-%m-%d",output.dayFormat="%Y-%m-%d",output.hourFormat="%Y-%m-%d %H:%M:%S")

# updating Enddate
subTime <- timeCheck(data=dailyDiary,ID="ID",day="ActivityDate",hour="Enddate",printInfo = FALSE,input.dayFormat="%Y-%m-%d",output.dayFormat="%Y-%m-%d",output.hourFormat="%Y-%m-%d %H:%M:%S")
subTime <- subTime[order(subTime$ID,subTime$Startdate),] # sorting by Startdate (now sorted by Enddate)
dailyDiary$Enddate <- subTime$Enddate
```
...
```{r message=FALSE,fig.width=10,fig.height=4,error=F}
(dailyDiary_compliance <-  
  timeCheck(data=dailyDiary,ID="ID",day="ActivityDate",hour="Startdate",returnInfo=TRUE,printInfo=FALSE, input.dayFormat="%Y-%m-%d",output.dayFormat="%Y-%m-%d",output.hourFormat="%Y-%m-%d %H:%M:%S"))
...
```{r message=FALSE,fig.width=10,fig.height=4, error=F}
dailyDiary$IDhour <- as.factor(paste(dailyDiary$ID,dailyDiary$Startdate,sep="_"))
dup <- dailyDiary[duplicated(dailyDiary$IDhour),c("ID","ActivityDate","Startdate","IDhour")]
cat("Detected",nrow(dup),"cases of double responses recorded between",
    as.character(dup[dup$Startdate==min(dup$Startdate),"Startdate"]),"and",
    as.character(dup[dup$Startdate==max(dup$Startdate),"Startdate"]))

dailyDiary[dailyDiary$IDhour%in%levels(as.factor(as.character(dup$IDhour))),
           c("ID","Startdate","Enddate",colnames(dailyDiary)[c(3,10,11)])]
```
...
- each duplicated case consists of two responses with the **same `Startdate` and `Enddate` values**
...
```{r message=FALSE,fig.width=10,fig.height=4, error=T}
# printing info
cat(nrow(dailyDiary[is.na(dailyDiary$Enddate),]),"cases with missing Enddate and",
...
dailyDiary[dailyDiary$CompletionStatus=="Partially Completed",
           c("ID","Startdate","Enddate","CompletionStatus",colnames(dailyDiary)[c(3,10,11)])]
```
...
- in **13 cases** the `CompletionStatus` is `"Partially Completed"`, and the `Enddate` is missing
...
dailyDiary$surveyDuration <- as.numeric(difftime(dailyDiary$Enddate,dailyDiary$Startdate,units="min"))
...
dailyDiary[dailyDiary$CompletionStatus=="Partially Completed","Enddate"] <-
  dailyDiary[dailyDiary$CompletionStatus=="Partially Completed","Startdate"] +
...
- in 17 cases the `surveyDuration` was longer than 15 min, among which **6 cases that submitted the responses more than 16h after the** `Startdate`*** `
...
# durations > 15 min (17)
dailyDiary[dailyDiary$surveyDuration > 15,c("ID","Startdate","Enddate","surveyDuration")]
...
Then, to better inspect timing and duration of the recorded sleep periods, we recode the `Startdate` and `Enddate` variables to create `StartHour` and `EndHour`, indicating only the time (and not the date). Note that `Startdate` and `Enddate` have a minute resolution (not seconds).
```{r warning=FALSE,message=FALSE}
dailyDiary <- StartTime_rec(data=dailyDiary,start="Startdate",end="Enddate",doPlot=TRUE,returnData=TRUE)
```
...
- most diaries were responded between **9 PM** (i.e., 1h after they were received) and **midnight**. Note that most `SartTime` values derived from the Fitabase `sleepLog` data were later than Survey Sparrow `Startdate`, which looks fine
...
cat(nrow(dailyDiary[dailyDiary$StartHour>=h00 & dailyDiary$StartHour<h20,c("ID","Startdate","StartHour","ActivityDate")]),
...
# showing duplicates
dailyDiary <- dailyDiary[order(dailyDiary$ID,dailyDiary$Startdate),] # re-sorting by ID and time
...
dailyDiary[dailyDiary$IDday%in%levels(as.factor(as.character(dupl$IDday))),c("IDday","Startdate","Enddate",
...
Here, for each of these groups of duplicated cases, **we only keep the first case** (i.e., the one with the earlier `Startdate`), whereas we **exclude 139 (3%) double responses**.
```{r }
# excluding double responses (keeping only the first one)
memory <- dailyDiary
...
We can also use the updated `ActivityDate` to check the No. of cases with **`Startdate` after `sleepLog` `EndTime` (N = 739)** (i.e., surveys answered on the following day), and the differences between the two time points (ranging from 0.5 min to 15.8h)
```{r warning=FALSE,message=FALSE, error=F}
# checking No. of cases with Startdate after wake up time (739)
dailyDiary$IDday <- as.factor(paste(dailyDiary$ID,dailyDiary$ActivityDate,sep="_"))
diaryAndsleep <- na.omit(plyr::join(dailyDiary[,c("IDday","Startdate")],sleepLog[,c("IDday","EndTime")],by="IDday",type="left"))
cat(nrow(diaryAndsleep[diaryAndsleep$Startdate>diaryAndsleep$EndTime,c("Startdate","EndTime")]),
    "cases in which participants started filling the diary after they woke up \n(",
    round(100*nrow(diaryAndsleep[diaryAndsleep$Startdate>diaryAndsleep$EndTime,])/nrow(diaryAndsleep),1),
    "% of cases with matching ID and ActivityDate between dailyDiary and sleepLog)")

# summarizing differences between sleepLog EndTime and dailyDiary Startdate in these 739 cases
summary(as.numeric(difftime(diaryAndsleep[diaryAndsleep$Startdate>diaryAndsleep$EndTime,"Startdate"],
                            diaryAndsleep[diaryAndsleep$Startdate>diaryAndsleep$EndTime,"EndTime"],units="mins")))
```
...
In the San Francisco area, the **Daylight Saving Time** (DST) changed on March 10th (1h forward) and November 3rd, 2019 (1h backward), and again on March 8th (1h forward) and November 1st, 2020 (1h backward), and finally on March 14th, 2021. Here, we inspect the distributions of `StartHour` values in the 5 days preceding and the 5 days following each of these dates, in order to check whether time was automatically updated by the wristband.
```{r fig.width=12,fig.height=4, error=F}
# setting DST changing times
DST.changes <- as.Date(c("2021-03-14","2021-11-07", "2022-03-13","2022-11-06","2023-03-12","2023-11-05"))
...
for(i in 1:length(DST.changes)){
  DST <- rbind(DST,dailyDiary[difftime(dailyDiary$ActivityDate,DST.changes[i],units="days")>(-5) &
                        ...
                        c("ID","ActivityDate","Startdate","StartHour","Enddate","EndHour","surveyDuration")])}
DST$DST <- FALSE # marking DST time changes with DST = TRUE
DST[DST$ActivityDate%in%DST.changes,"DST"] <- TRUE

# computing time (hours) from midnight
DST$timeFrom00 <- as.POSIXct(paste(lubridate::hour(DST$Startdate), lubridate::minute(DST$Startdate)), format="%H %M") 
DST$timeFrom00 <- as.numeric(difftime(DST$timeFrom00,
                                      as.POSIXct(paste(substr(Sys.time(),1,10),"00:00:00"),tz="GMT"),units="hours"))
# subtracting 1 day to cases with timeFrom00 > 12
DST[DST$timeFrom00>12,"timeFrom00"] <- DST[DST$timeFrom00>12,"timeFrom00"] - 24
...
- the visual inspection of `Startdate` trends in those participants that recorded their sleep during the days around DST changes **does not seem to suggest systematic shifts** pairing with time changes

- the only DST change that shows some substantial shift in `Startdate` is the third one (2019-03-10), with four participants out of five showing an increasing upward trend of five-to-ten hours on the following day
...
Finally, we plot epochs order against `Time` for each participant in order to better inspect the pattern of missing data.
```{r message=FALSE,error=FALSE,fig.width=10,fig.height=10, error=F}
par(mfrow=c(3,3))
for(ID in levels(dailyDiary$ID)){ 
  plot((1:nrow(dailyDiary[dailyDiary$ID==ID,])~dailyDiary[dailyDiary$ID==ID,"Startdate"]),main=ID,xlab="",ylab="") }
```
...
```{r fig.width=10,fig.height=4, error=F}
# removing variables
toRemove <- c("IDday","IDhour","StartHour","EndHour")
dailyDiary[,toRemove] <- NULL

# renaming variables
colnames(dailyDiary)[which(colnames(dailyDiary)=="Howstressfulwasyourday"
                           ):which(colnames(dailyDiary)=="AreyouworriedaboutanyofthefollowingchoiceotheregIamworriedaboutsomethingelsehappeningtomorrow")] <-
  c("dailyStress","stress_school","stress_family","stress_health","stress_peers","stress_other",
    "eveningMood",
    "eveningWorry","worry_school","worry_family","worry_health","worry_peer","worry_sleep","worry_other") 
```
...
dailyDiary$dailyStress <- as.numeric(gsub("Not at all stressful","0",
                                          gsub("Not so stressful","1",
                                               gsub("Somewhat stressful","2",
                                                    gsub("Very stressful","3",
                                                         gsub("Extremely stressful","4",
                                                              dailyDiary$dailyStress))))))
# converting dailyStress sources as binary (0/1)

dailyDiary <- dailyDiary %>%
  dplyr::mutate(across(starts_with("stress_"), ~ifelse(is.na(.), 0, ifelse(. == "Unchecked", 0, 1)))) 
...
dailyDiary$eveningWorry <- as.numeric(gsub("Not at all worried","0",
                                          gsub("Not so worried","1",
                                               gsub("Somewhat worried","2",
                                                    gsub("Very worried","3",
                                                         gsub("Extremely worried","4",
                                                              dailyDiary$eveningWorry))))))
# converting eveningWorry sources as binary (0/1)
dailyDiary <- dailyDiary %>%
  dplyr::mutate(across(starts_with("worry_"), ~ifelse(is.na(.), 0, ifelse(. == "Unchecked", 0, 1)))) 
...
dailyDiary$eveningMood <- as.numeric(gsub("Very bad","0",
                                          gsub("Somewhat bad","1",
                                               gsub("Neither bad or good","2",
                                                    gsub("Somewhat good","3",
                                                         gsub("Very good","4",
...
```{r , error=F }
# sorting variables
dailyDiary <- dailyDiary[,c("ID","ActivityDate","Startdate","Enddate","surveyDuration",
...
demos <- read.csv("~/Documents/INSA-home/Appendix B - Data pre-processing/DATA/demographics.csv") # demos

# recoding participants' identifier
demos$ID <- as.factor(demos$ID)
demos$sex <- as.character(demos$sex)


# changing variable classes
demos$sex <- gsub("0","Female",gsub("1","Male",demos$sex))
demos[,c("sex","insomnia","insomnia.group")] <- 
  lapply(demos[,c("sex","insomnia","insomnia.group")],as.factor)
demos[,c("age","BMI")] <- lapply(demos[,c("age","BMI")],as.numeric) 

# sorting variables and removing unuseful columns
#demos <- demos[,c("ID","sex","age","BMI","insomnia","insomnia.group")] # sorting columns

# plotting variables
par(mfrow=c(2,3)) 
for(Var in c(colnames(demos)[2:6])){ 
...
demos = demos %>%
  dplyr::mutate(insomnia.group = ifelse(!is.na(sub_insomnia) & sub_insomnia==1, "sub.ins", "control")) # sub.ins when sub_insomnia = 1
...
LogId <- as.character(sleepLog[sleepLog$combined==FALSE & sleepLog$missing_start==0 & sleepLog$nFinalWake==0 & sleepLog$TimeInBed - sleepLog$TIB > 60,"LogId"])
...
if (nrow(specialCases)){
  sleepLog.uniqueEBE_eval = T
...
diff <- sleepLog[abs(sleepLog$WASO_rec - sleepLog$WASO) > abs(0.5),"WASO_rec"] - 
  sleepLog[abs(sleepLog$WASO_rec - sleepLog$WASO) > abs(0.5),"WASO"]
...
```{r eval = sleepLog.uniqueEBE_eval, fig.width=10,fig.height=4,warning=FALSE, error = T}
# showining 44 cases
...
```{r eval = sleepLog.uniqueEBE_eval, error = T, fig.width=10,fig.height=3,warning=FALSE}
...
if (nrow(sleepLog[sleepLog$IDday %in% sleepLog$IDday[duplicated(sleepLog$IDday)],]) > 2){
...
Here, we create the variable `IDday` to be used for merging the `dailyDiary` and the `fitbit` datasets. Note that `dailyDiary` `ActivityDate` was recoded to be referred to the previous day when `Startdate` is between midnight and 8 PM (see section 2.7.5).
```{r }
# creating common variable IDday
dailyDiary$IDday <- as.factor(paste(dailyDiary$ID,dailyDiary$ActivityDate,sep="_"))
...
Then, we inspect the cases with `dailyDiary` `Startdate` value (i.e., the time at which the survey was responded) and the corresponding `sleepLog StartTime` values (i.e., lights-off time according to `sleepLog` data).
```{r fig.width=10,fig.height=3, error=F}
ema$diary.timeDiff <- as.numeric(difftime(ema$StartTime,ema$Startdate,units="hours"))
hist(ema$diary.timeDiff,breaks=100,main="hours between dailyDiary Startdate and sleepLog StartTime")

# showing 1,000 cases with dailyDiary Startdate AFTER sleepLog StartTime
ema[!is.na(ema$diary.timeDiff) & ema$diary.timeDiff<0,c("ID","ActivityDate","Startdate","StartTime","EndTime")]
```
...
- in most cases (N = 3,402, 77%) `dailyDiary` `Startdate` is **before** `sleepLog StartTime`

- in 1,000 cases (**23%**) `dailyDiary Startdate` is **after** `sleepLog` `StartTime`, above we can see that most of these cases are cases in which the **diary was responded on the following day**
...
compliance <- compliance %>%
  mutate(nSleepStage_clean = sapply(ID, function(id) {
    nlevels(factor(Nsleep[as.character(Nsleep$ID) == id & !is.na(Nsleep$light), "IDday"]))
  }))
...
# computing No. of cases with nonmissing sleep stage data
compliance <- compliance %>%
  mutate(nSleepHR = sapply(ID, function(id) {
    nlevels(factor(Nsleep[as.character(Nsleep$ID) == id & !is.na(Nsleep$stageHR_NREM), "IDday"]))
  }))
...
for (sleepHRVar in sleepHRVars) {
  # Updating compliance dataset
  ema <- ema %>%
    rename_with(~ "sleepHR", sleepHRVar)

  # Computing compliance_clean (reference: 60 days)
  compliance[[sleepHRVar]] <- ema %>%
    filter(!is.na(sleepHR)) %>%
    group_by(ID) %>%
    summarize(nSleepHR = n_distinct(IDday)) %>%
    pull(nSleepHR)

  # Printing compliance info
  cat("\n\n", sleepHRVar, ":\n- 'Cleaned' No. days/participants =",
      round(mean(compliance[[sleepHRVar]]), 2), " ( SD =", round(sd(compliance[[sleepHRVar]]), 2),
      ") \n- compliance =", round(mean(100 * compliance[[sleepHRVar]] / 60), 2), "% ( SD =",
      round(sd(100 * compliance[[sleepHRVar]] / 60), 2),
      ") \n- non-missing", sleepHRVar, "/Total No. 'clean' sleepLog cases =",
      round(mean(100 * compliance[[sleepHRVar]] / compliance$nSleep_clean), 2),
      "% ( SD =", round(sd(100 * compliance[[sleepHRVar]] / compliance$nSleep_clean), 2), ")")

  # Plotting No. of cases
  hist(compliance[[sleepHRVar]], breaks = 50,
       main = paste("No. of nonmissing", sleepHRVar, "values per participant"), xlab = "")

  # Back to the original variable name
  ema <- ema %>%
    rename_with(~ sleepHRVar, all_of("sleepHR"))

  colnames(compliance)[which(colnames(compliance) == sleepHRVar)] <- paste("n", sleepHRVar, "_clean", sep = "")
}
...
compliance <- compliance %>%
  mutate(ndailyAct = sapply(compliance$ID, function(id) {
    nlevels(factor(ema[as.character(ema$ID) == id & !is.na(ema$TotalSteps), "IDday"]))
  })) %>%
    mutate(ndailyAct_sleep = sapply(compliance$ID, function(id){ 
    nlevels(as.factor(as.character(ema[(ema$ID)== id  & !is.na(ema$TIB) & !is.na(ema$TotalSteps),"IDday"])))
  }))
...
compliance <- compliance %>%
  mutate(ndailyAct_clean = sapply(compliance$ID, function(id){ 
    nlevels(as.factor(as.character(Nact[as.character(Nact$ID)==id,"IDday"])))})) %>% 
  mutate(ndailyAct_sleep_clean = sapply(compliance$ID, function(id){ 
    nlevels(as.factor(as.character(Nact[as.character(Nact$ID)==id & 
...
```{r }
cat(nrow(ema[!is.na(ema$Startdate),]),"nonmissing cases of dailyDiary data") # 4,945
```
...
Ndiary <- ema[!is.na(ema$Startdate),]
for(i in 1:nrow(compliance)){
  compliance[i,"nDiary"] <- nlevels(as.factor(as.character(Ndiary[as.character(Ndiary$ID)==compliance[i,"ID"],
                                                                  "IDday"])))
  compliance[i,"periodDiary"] <- 
    difftime(max(Ndiary[as.character(Ndiary$ID)==compliance[i,"ID"],"Startdate"]),
             min(Ndiary[as.character(Ndiary$ID)==compliance[i,"ID"],"Startdate"]),
             units="days")
  compliance[i,"nDiary_sleep"] <- 
    nlevels(as.factor(as.character(ema[as.character(ema$ID)==compliance[i,"ID"] & 
                                         !is.na(ema$TIB) & !is.na(ema$Startdate),"IDday"])))
  compliance[i,"nDiary_sleepAct"] <- 
    nlevels(as.factor(as.character(ema[as.character(ema$ID)==compliance[i,"ID"] & 
               !is.na(ema$TIB) & !is.na(ema$TotalSteps) & !is.na(ema$Startdate),"IDday"]))) }
...
- **43 double responses** with the same `Startdate` values **were excluded**
...
```{r message=FALSE,warning=FALSE, fig.width=11,fig.height=4, error=F}
# computing LAG values for ActivityDate
library(dplyr)
Ndiary <- Ndiary[order(Ndiary$ID,Ndiary$ActivityDate,Ndiary$Startdate),]
...
Ndiary.vars <- c("ID","ActivityDate","lag",
                 colnames(ema)[which(colnames(ema)=="Startdate"):which(colnames(ema)=="diary.timeDiff")])
...
if (nrow(isolatedDay) > 0){
  isolatedDay[,c("ID","ActivityDate","lag","Startdate","dailyStress","eveningMood","TIB","TotalSteps")]
}
```
...
- **only one** of these cases (`ID` s052, `Startdate` 2019-12-01 21:09:00	) is due nly due to **isolated final day** recorded several days later than the previous observation from the same participant, with **no corresponding `sleepLog` or `dailyAct` values**
...
diaryVars <- colnames(ema)[which(colnames(ema)=="Startdate"):which(colnames(ema)=="diary.timeDiff")]

# manually removing dailyDiary variables from 1 case
memory <- ema
ema[!is.na(ema$Startdate) & ema$ID=="s052" & as.character(ema$Startdate)=="2019-12-01 21:09:00", 
    diaryVars] <- NA

# print info (10 removed cases)
cat("No. excluded cases of isolated days =",
    nrow(memory[!is.na(memory$Startdate),])-nrow(ema[!is.na(ema$Startdate),]))
```
...
```{r fig.width=12,fig.height=3,warning=FALSE,error=F}
# printing info
Ndiary <- ema[!is.na(ema$Startdate),]
n <- nrow(Ndiary)
...
Ndiary[is.na(Ndiary$dailyStress) | is.na(Ndiary$eveningWorry) | is.na(Ndiary$eveningMood),
       c("ID","Startdate","dailyStress","eveningMood","eveningWorry","TIB","TotalSteps","stageHR_NREM")]
```
...
memory <- ema
ema[!is.na(ema$Startdate) & (is.na(ema$dailyStress) | is.na(ema$eveningWorry) | is.na(ema$eveningMood)),
    diaryVars] <- NA
cat("Removed",nrow(ema[is.na(ema$Startdate),])-nrow(memory[is.na(memory$Startdate),]),"cases")
```
<br>

Finally, we inspect the No. of missing data in the remaining `dailyDiary` variables.
```{r error=F}
summary(ema[!is.na(ema$Startdate),diaryVars])
```
...
```{r error=F}
cat(nrow(ema[!is.na(ema$Startdate),]),"'cleaned' cases of dailyDiary data") # 4,932
```

<br>

Here, compute the updated information on the nonmissing data related to `dailyDiary` variables.
```{r fig.width=11,fig.height=3, error=F}
# updating compliance dataset
Ndiary <- ema[!is.na(ema$Startdate),]
for(i in 1:nrow(compliance)){
...
cat("\n\nsleepLog data: total No. 'clean' cases =",nrow(ema[!is.na(ema$TIB),]), # sleepLog
    ...
    ")\n\ndailyDiary data: total No. 'clean' cases =",nrow(ema[!is.na(ema$Startdate),]), # dialyDiary
...
cat("\n\nsleepLog data: total No. 'clean' cases =",nrow(n),", of which:",
    ...
    "% )\n- ",nrow(n[!is.na(n$Startdate),]),"cases with nonmissing dailyDiary data (",
    round(100*nrow(n[!is.na(n$Startdate),])/nrow(n),2),
...
tempCont <- function(data=NA,compliance=NA){ 
  ...
  data <- data[!(is.na(data$TIB) & is.na(data$Startdate)),]
  
  par(mfrow=c(3,3))
  for(ID in levels(data$ID)){ 
    IDdata <- data[data$ID==ID,]
    IDcompliance <- compliance[compliance$ID==ID,]
    max_compliance = max(IDcompliance$nSleep_clean,IDcompliance$nSleepStage_clean,IDcompliance$nstageHR_NREM_clean,IDcompliance$nDiary_clean,IDcompliance$ndailyAct_clean)
    max_compliance = ifelse(is.na(max_compliance),0,max_compliance)
    
    Xlim <- c(min(IDdata[IDdata$ID==ID,"ActivityDate"])-30,max(IDdata[IDdata$ID==ID,"ActivityDate"])+30)
    Ylim <- c(0,max_compliance)
  n <- IDdata[!is.na(IDdata$TotalSteps),] # yellow = dailyAct
  if(nrow(n[n$ID==ID,])>0){ 
    
    n$ActivityDate <- n$ActivityDate - 10
  ...
  # n <- IDdata[!is.na(IDdata$Startdate),]; n$ActivityDate <- n$ActivityDate - 20 # green = dailyDiary
  # points((1:nrow(n[n$ID==ID,])~n[n$ID==ID,"ActivityDate"]),main=ID,xlab="",ylab="",xlim=Xlim,ylim=Ylim,
  ...
  abline(h=ifelse(IDcompliance$ndailyAct_clean > 0,IDcompliance$nDiary_sleepAct_clean,
    ...
    n <- IDdata[!is.na(IDdata$TIB),]
    
    n$ActivityDate <- n$ActivityDate
...
```{r message=FALSE,error=F,fig.width=11,fig.height=11}
...
ema <- ema[,c("ID","sex","age","BMI","insomnia","insomnia.group", # demos at the beginning
              ...
              # dailyDiary
              "Startdate","Enddate","surveyDuration",
              "dailyStress","eveningWorry","eveningMood",
...
31. `Startdate` = initiation hour of the diary form (in "mm-dd-yyyy hh:mm:ss" format)

32. `Enddate` = submission hour of the diary form (in "mm-dd-yyyy hh:mm:ss" format)
